...
grep -nE '^_rm\(\)|safe_remove_path "\$@"|^_cp\(\)|safe_copy_file|safe_copy_contents|^_sed\(\)|safe_sed_inplace|^_mkdir\(\)|safe_ensure_dir' "$REPO_ROOT/src/init.sh" >"$TEST_ROOT/legacy-helper-routing.out"
...
if grep -nE 'tar zxf "\$tmpfile" --strip-components 1 -C "\$is_core_dir/bin"' "$REPO_ROOT/src/download.sh" >/dev/null; then
...
if grep -nE 'tar zxf "\$is_core_ok" --strip-components 1 -C "\$is_core_dir/bin"' "$REPO_ROOT/install.sh" >/dev/null; then
...
if grep -nE '\[\[ ! -d \$is_caddy_conf \]\] && mkdir -p \$is_caddy_conf' "$REPO_ROOT/src/core.sh" >/dev/null; then
    risk P1 src/core.sh 'create caddy' 'Caddy conf directory can be created with raw mkdir -p instead of safe_ensure_dir'
fi
if grep -nE 'mkdir -p \$is_caddy_dir \$is_caddy_dir/sites \$is_caddy_conf' "$REPO_ROOT/src/caddy.sh" >/dev/null; then
...
if grep -nE 'cat <<<.*>\$is_config_json' "$REPO_ROOT/src/dns.sh" "$REPO_ROOT/src/log.sh" >/dev/null; then
...
if grep -nE '>\$is_tls_(tmp|key|cer)|rm \$is_tls_tmp' "$REPO_ROOT/src/init.sh" >/dev/null; then
...
if grep -nE 'sed -i .* /etc/sysctl\.conf|>>/etc/sysctl\.conf' "$REPO_ROOT/src/bbr.sh" >/dev/null; then
    risk P2 src/bbr.sh '_open_bbr' 'BBR modifies /etc/sysctl.conf without backup transaction; outside sing-box managed-file scope'
fi
if grep -nE 'tar zcvf code\.tar\.gz \./\*' "$REPO_ROOT/.github/workflows/release.yml" >/dev/null; then
