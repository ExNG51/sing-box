...
allow_ufw_udp_443() {
    # 中文注释：UFW allow 规则相对幂等，重复执行不应破坏现有策略。
    ufw allow 443/udp >/dev/null 2>&1 || err "UFW 放行 UDP 443 失败。"
    _green "已通过 UFW 放行 UDP 443。"
}

allow_firewalld_udp_443() {
    local runtime_allowed=false
    local permanent_allowed=false

    # 中文注释：firewalld 必须同时保证 runtime 与 permanent；runtime-only 状态需要补 permanent 并 reload。
    firewall-cmd --query-port=443/udp >/dev/null 2>&1 && runtime_allowed=true
    firewall-cmd --permanent --query-port=443/udp >/dev/null 2>&1 && permanent_allowed=true

    if [[ $runtime_allowed == true && $permanent_allowed == true ]]; then
        _green "firewalld 已放行 UDP 443。"
        return 0
    fi

    if [[ $permanent_allowed != true ]]; then
        firewall-cmd --permanent --add-port=443/udp >/dev/null 2>&1 || {
            err "firewalld 添加 UDP 443 永久规则失败。"
        }
    fi
    ...
    _green "已通过 firewalld 放行 UDP 443。"
}
...
warn_manual_firewall_udp_443() {
    local backend=$1

    warn "检测到 $backend，但脚本不自动修改复杂底层防火墙规则。请确认 UDP 443 入站已放行。"

    case "$backend" in
    nftables)
        msg "参考检查命令：nft list ruleset"
        msg "请按当前 ruleset 结构添加 udp dport 443 accept。"
        ;;
    iptables)
        msg "临时放行参考：iptables -C INPUT -p udp --dport 443 -j ACCEPT || iptables -I INPUT -p udp --dport 443 -j ACCEPT"
        msg "注意：iptables 持久化方式取决于发行版。"
        ;;
    esac

    if [[ $is_main_start ]]; then
        ask string y "我已确认 UDP 443 入站已放行 [y]:"
    fi
}
...
ensure_udp_443_firewall() {
    ...
    firewalld)
        ...
        ;;
    nftables | iptables)
        warn_manual_firewall_udp_443 "$firewall_backend"
        ;;
    none)
        msg "$is_warn 未检测到已启用的 UFW/firewalld；跳过 UDP 443 本机防火墙规则修改。"
        ;;
    *)
        warn "无法识别防火墙类型，请手动确认 UDP 443 入站已放行。"
        ;;
    esac
}

warn_udp_443_external_firewall() {
    msg "$is_warn 如果服务器位于 AWS / Lightsail / Oracle / Azure / GCP / 阿里云 / 腾讯云等平台，请同时确认云安全组已放行 UDP 443。"
}
